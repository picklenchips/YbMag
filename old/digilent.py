...
Referenced [Waveforms-SDK](https://github.com/Digilent/WaveForms-SDK-Getting-Started-PY/) examples.
"""
...
class Digilent:
    ...
    def _call_dwf(self, func_name: str, *args) -> None:
        """Invoke a DWF API function and raise with context on failure."""
        fn = getattr(self._dwf, func_name)
        ok = int(fn(*args))
        if ok != 0:
            return
        try:
    ...
    def open(self, device_index: Optional[int] = None) -> None:
        ...
        with self._lock:
            ...
            self._call_dwf(
                "FDwfDeviceOpen", ctypes.c_int(idx), ctypes.byref(self._hdwf)
            )
    ...
    def close(self) -> None:
        ...
        with self._lock:
            if self._connected:
                self._call_dwf("FDwfDeviceClose", self._hdwf)
                self._hdwf.value = 0
                self._connected = False
                self._running = False
    ...
    def _configure_digital_channel_locked(self, config: DigitalChannelConfig) -> None:
        ...
        self._call_dwf(
            ...
            self._hdwf,
            ch,
            ctypes.c_int(_DWFDIGITALOUT_TYPE_PULSE),
        )
        ...
        self._call_dwf("FDwfDigitalOutIdleSet", self._hdwf, ch, ctypes.c_int(idle))
        ...
        self._call_dwf("FDwfDigitalOutDividerSet", self._hdwf, ch, ctypes.c_uint(1))
        self._call_dwf(
            "FDwfDigitalOutCounterSet",
            self._hdwf,
            ch,
            ctypes.c_uint(low_ticks),
            ctypes.c_uint(high_ticks),
        )
    ...
    def configure_all_digital(self, configs: List[DigitalChannelConfig]) -> None:
        ...
        with self._lock:
            self._require_connected()
            self._call_dwf("FDwfDigitalOutReset", self._hdwf)
            for cfg in configs:
    ...
    def set_repeat_count(self, count: int = 0) -> None:
        ...
        with self._lock:
            self._require_connected()
            self._call_dwf("FDwfDigitalOutRepeatSet", self._hdwf, ctypes.c_int(count))
    ...
    def set_run_duration(self, seconds: float = 0.0) -> None:
        ...
        with self._lock:
            self._require_connected()
            self._call_dwf("FDwfDigitalOutRunSet", self._hdwf, ctypes.c_double(seconds))
    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            # Re-apply global digital-out settings because DigitalOutReset can
            # clear these and prevent expected output generation.
            self._call_dwf(
            ...
            # Repeat-trigger only applies when using an external trigger source.
            ...
            self._call_dwf("FDwfDigitalOutConfigure", self._hdwf, ctypes.c_int(1))
            self._running = True
    ...
    def stop(self) -> None:
        ...
        with self._lock:
            if not self._connected:
                self._running = False
                return
            self._call_dwf("FDwfDigitalOutConfigure", self._hdwf, ctypes.c_int(0))
            self._running = False
    ...
    def trigger(self) -> None:
        ...
        with self._lock:
            self._require_connected()
            self._call_dwf("FDwfDeviceTriggerPC", self._hdwf)
    ...
    def is_running(self) -> bool:
        ...
        with self._lock:
            if not self._connected:
                return False
            sts = ctypes.c_int()
            self._call_dwf("FDwfDigitalOutStatus", self._hdwf, ctypes.byref(sts))
    ...
    def configure_scope_channel(self, config: ScopeChannelConfig) -> None:
        ...
        with self._lock:
            ...
            self._call_dwf(
                "FDwfAnalogInChannelFilterSet", self._hdwf, ch, ctypes.c_int(coupling)
            )
    ...
    def configure_scope_trigger(
        ...
        with self._lock:
            ...
            self._call_dwf(
                "FDwfAnalogInTriggerSourceSet", self._hdwf, ctypes.c_int(src)
            )
            self._call_dwf(
                "FDwfAnalogInTriggerChannelSet", self._hdwf, ctypes.c_int(channel)
            )
            self._call_dwf(
                "FDwfAnalogInTriggerLevelSet", self._hdwf, ctypes.c_double(level_volts)
            )
            cond = _TRIGCOND_RISING if rising else _TRIGCOND_FALLING
            self._call_dwf(
                "FDwfAnalogInTriggerConditionSet", self._hdwf, ctypes.c_int(cond)
            )
            self._call_dwf(
    ...
    def start_scope(self) -> None:
        ...
        with self._lock:
            self._require_connected()
            self._call_dwf(
                "FDwfAnalogInConfigure", self._hdwf, ctypes.c_int(1), ctypes.c_int(1)
            )
    ...
    def poll_scope(self, channel: int = 0) -> Optional[ScopeAcquisition]:
        ...
        with self._lock:
            ...
            sts = ctypes.c_int()
            self._call_dwf(
                "FDwfAnalogInStatus", self._hdwf, ctypes.c_int(1), ctypes.byref(sts)
            )
    ...
    def stop_scope(self) -> None:
        ...
        with self._lock:
            if not self._connected:
                return
            self._call_dwf(
                "FDwfAnalogInConfigure", self._hdwf, ctypes.c_int(0), ctypes.c_int(0)
            )
...
if __name__ == "__main__":
    ...
    def _prompt_sine_wave() -> Optional[Tuple[float, float]]:
        while True:
            try:
                freq_hz = _prompt_float("D0 frequency (Hz)", 1000.0)
                if freq_hz is None:
                    return None
                amplitude = _prompt_float("D0 amplitude (V)", 2.5)
    ...
    d = Digilent()
    d.open()
    d.set_trigger_source(TRIGSRC_NONE)
    d.set_repeat_count(0)
    d.set_run_duration(0.0)
    try:
        # main loop
        while True:
            function = input("S for square wave, W for sine wave:").strip().lower()
            if function == "q":
                break
            # square wave
            if function in ["s", ""]:
                # just do a square wave
                out = _prompt_square_wave()
                if out is None:
                    print("Exiting.")
                    exit(0)
                freq_hz, duty_pct = out
                freq_hz = max(1e-3, float(freq_hz))
                duty_cycle = min(0.99, max(0.01, float(duty_pct) / 100.0))
                d.configure_all_digital(
                    [
                ...
                print(
                    f"Starting D{channel} at {freq_hz:.2f} Hz, duty {duty_cycle * 100:.1f}%"
                )
            elif function == "w":
                print("Sine wave has not been implemented yet! ")
                continue
                out = _prompt_sine_wave()
                if out is None:
                    print("Exiting.")
                    exit(0)
                freq_hz, amplitude = out
                freq_hz = max(1e-3, float(freq_hz))
                amplitude = max(0.0, min(5.0, float(amplitude)))

            d.start()
            code = (
                input(
                    "Running. Press Enter to stop and capture scope, or 'q' to quit: "
                )
                .strip()
                .lower()
            )
